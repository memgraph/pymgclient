...
def test_string(memgraph_connection):
    ...
    cursor.execute(
        "RETURN $value", {"value": b"\x01\x0C\x01\x7D\x01\x60".decode("utf-16be")}
    )
    assert cursor.fetchall() == [("ČŽŠ",)]
...
def test_map(memgraph_connection):
    ...
    result = cursor.fetchall()
    assert result == [
        ({"x": 1, "y": 2, "map": {key_in_a_map: "value"}, "list": value_in_a_map},)
    ]
...
def test_node(memgraph_connection):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "CREATE (n:Label1:Label2 {prop1 : 1, prop2 : 'prop2'}) " "RETURN id(n), n"
    )
    ...
    assert rows == [
        (
            node_id,
            mgclient.Node(
                node_id, set(["Label1", "Label2"]), {"prop1": 1, "prop2": "prop2"}
            ),
        )
    ]
...
def test_path(memgraph_connection):
    ...
    assert rows == [
        (n1_id, n2_id, n3_id, e1_id, e2_id, mgclient.Path([n1, n2, n3], [e1, e2]))
    ]
...
def test_datetime(memgraph_connection):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "RETURN $value", {"value": datetime.datetime(2004, 7, 11, 12, 13, 14, 15)}
    )
    result = cursor.fetchall()
